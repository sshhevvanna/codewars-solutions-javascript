...
- The flag couldn't be found from the string literal right away... but _You can indeed **see** it somehow!_
...
for (let i = 0; i < FLAG.length; i += 16) {
    const string = FLAG.slice(i, i + 16);
    array.push(string);
  }

console.log(array); // now you see the flag in the console (not codewars one, use browser)
